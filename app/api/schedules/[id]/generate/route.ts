...
export async function POST(
  ...
  try {
    ...
    if (!dbUser || !["SUPER_ADMIN", "ADMIN", "PATHFIT"].includes(dbUser.role)) {
      return NextResponse.json(
        apiError(
          dbUser?.role === "NSTP"
            ? "NSTP is scheduled manually — add NSTP classes with Add Entry"
            : "Only Department Chairpersons, Program Chairpersons and the PATHFit Director can run schedule generation"
        ),
        { status: 403 }
      )
    }
    ...
    // ── PATHFit coordinator: a dedicated placeholder-only run ───────────────
    // Places PATHFit for every section of this schedule's department (TBA
    // faculty, GYM). Nothing else in this route applies to that account.
    if (dbUser.role === "PATHFIT") {
      const outcome = await runPathfitGeneration(id, dbUser)
      if (!outcome.ok) {
        return NextResponse.json(
      ...
      return NextResponse.json(apiResponse(outcome.body))
    }
    ...
    // NSTP/PATHFIT are never part of a Dept Chair run (see EXCLUDED_AUTO below).
    ...
    // Prefixes kept out of this pipeline entirely. NSTP is scheduled manually by
    // the NSTP coordinator; PATHFit is generated by the PATHFIT coordinator's own
    // run (lib/services/pathfit-generation.ts) — never by a Dept / Program Chair.
    ...
    // manual entry — lib/services/entry-validation.ts — applies the same rule.)
    const termLabel = `${schedule.semester?.type === "FIRST" ? "1st" : schedule.semester?.type === "SECOND" ? "2nd" : "Summer"} Semester`

    // Instructors allocated to this program for this term by the Department
    ...
    const allocatedFacultyIds: string[] = isAdmin && programId
      ? (
          await db.facultyRequest.findMany({
            where: { status: "APPROVED", programId, semesterId: schedule.semesterId, facultyId: { not: null } },
            select: { facultyId: true },
          })
        ).map((r: any) => r.facultyId as string)
      : []
    ...
    const [subjects, faculty, rooms, sections] = await Promise.all([
      ...
      db.faculty.findMany({
        where: {
          ...
          // ADMIN: their own program first (program-specific + dept-wide), plus any
          //   instructor the Department Chairperson allocated to this program for this term.
          ...(isSuperAdmin
            ? { department: { college: { abbreviation: "CAS" } } }
            : {
                OR: [
                  { departmentId: deptId, programId },
                  { departmentId: deptId, programId: null },
                  ...(allocatedFacultyIds.length ? [{ id: { in: allocatedFacultyIds } }] : []),
                ],
              }
          ),
        },
      ...
      db.room.findMany({
        ...
        where: {
          ...
          // assigned to PATHFIT by the PATHFIT coordinator's own run.
    ...
    // PATHFit is no longer part of the Dept Chair's run — the PATHFIT
    // coordinator account generates it (lib/services/pathfit-generation.ts) and
    // the early return above handles that role. NSTP is manual. Both prefixes
    // are already excluded from the subject scope via notAutoExcluded.
    const allSubjects: any[] = [...subjects]
    ...
    if (facultyWithAvailability.length === 0) {
      return NextResponse.json(
        {
          success: false,
          error: `No faculty availability has been recorded for the ${termLabel} of this schedule's academic year`,
          details: [
            `Open Faculty Availability, choose this term (${termLabel}) in the Term selector, and mark hours for at least one faculty member. Availability from another term is never used.`,
            `Faculty loaded: ${facultyInputs.length} (${isAdmin ? "own program + department-wide pool" + (allocatedFacultyIds.length ? ` + ${allocatedFacultyIds.length} allocated` : "") : "CAS college"})`,
          ],
        },
        { status: 422 }
      )
    ...
    console.log(
      ...
      `${subjectInputs.length} subjects (${isAdmin ? "major only, GEC/PATHFIT excluded" : "GEC/GEL (PATHFit is the coordinator's run)"}), ` +
    ...
    if (isSuperAdmin) {
      try {
        ...
        // other (same trick as lib/services/term-conflicts.ts). This run never
        // writes placeholder rows itself any more, but the other side may hold
        // the PATHFit coordinator's TBA/GYM entries.
        const tbaFacultyId: string | null = null
    ...
    const notifMessage = stagePrefix + (result.unassigned.length > 0
      ? `${assignedCount} classes scheduled (${entryRows.length} entries, ${labCount} labs). ${result.unassigned.length} could not be placed — see the Unassigned list.`
      : `Done. ${assignedCount} classes scheduled (${entryRows.length} entries, ${labCount} labs).`)
    ...
    await recordAudit({
      ...
      scheduleId: id,
      summary: `Generated ${isAdmin ? "major subjects" : "GEC/GEL"} for ${schedule.department?.abbreviation ?? "department"} — ${assignedCount} classes, ${result.unassigned.length} unplaced${conflictsDetected ? `, ${conflictsDetected} cross-department conflicts` : ""}`,
      metadata: {
        Mode: citLabsOnlyStage ? "CIT laboratory pre-plot" : isAdmin ? "Major subjects" : "GEC/GEL",
        Term: schedule.semester?.type ?? "",
        "Subjects in scope": subjectInputs.length,
        "Sections in scope": sectionInputs.length,
        "Classes placed": assignedCount,
        "Entries written": entryRows.length,
        "Could not be placed": result.unassigned.length,
        "Cross-department conflicts": conflictsDetected,
        "Took": `${((Date.now() - generationStartedAt) / 1000).toFixed(1)} s`,
      },
    })
