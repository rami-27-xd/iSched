...
export default function SignInPage() {
  ...
  // The OAuth callback bounces failures back here as ?error=... . Nothing read it,
  // so a failed Google round trip landed the user on a clean sign-in form with no
  // indication anything had gone wrong.
  useEffect(() => {
    const params = new URLSearchParams(window.location.search)
    const code = params.get('error')
    if (!code) return
    // 'provider_error' carries Google's own wording — show that rather than a guess.
    const description = params.get('error_description')
    setPwError(
      code === 'provider_error'
        ? description
          ? `Google sign-in was not completed: ${description}`
          : 'Google sign-in was cancelled or refused. Please try again.'
        : code === 'missing_code'
          ? 'Google did not return a sign-in code. Try again, and check that this site URL is listed under Supabase → Authentication → URL Configuration.'
          : code === 'auth_failed'
            ? 'Google sign-in could not be completed. The redirect URL for this deployment may not be allowed in Supabase → Authentication → URL Configuration.'
  ...
  // Google sign-in used to swallow every failure: a createClient() throw was
  // caught and discarded, the { error } from signInWithOAuth was ignored, and the
  // ?error= the callback redirects back with was never read. All three failure
  // modes looked identical to the user — a button that does nothing. Surface them.
  async function handleGoogleSignIn() {
    ...
    try {
      ...
      setPwError(envErr?.message ?? 'Authentication is not configured on this deployment.')
    ...
    if (error) {
      setPwError(
        error.message.toLowerCase().includes('provider is not enabled')
          ? 'Google sign-in is not enabled for this project yet. Enable the Google provider in Supabase → Authentication → Providers.'
          : error.message
      )
      setGoogleLoading(false)
      return
    }
    // On success the browser navigates away to Google; leave the spinner running.
  }
